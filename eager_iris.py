...
def keras_training():
    ...
    hist = model.fit(train_x, train_y, epochs=num_epochs, batch_size=batch_size)
    return hist
...
dataset_train = Dataset.from_tensor_slices((train_x, train_y)).batch(batch_size)
...
def eager_training():
    ...
    for epoch in range(num_epochs):
        ...
        for b, (x, y) in enumerate(dataset_train):
            print(b, y.shape)
            # Optimize the model
